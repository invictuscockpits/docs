...
**What you'll do:** wire a rotary switch to one analog input instead of one pin per position, then teach the manager what each position reads.
...
Nothing changes on the board. The board reports the voltage as a number, and the manager keeps the table of which number belongs to which position.

## How it works

![A chain of equal resistors between 3.3 V and GND, each switch position tapping a different point, the common going to the analog input](images/resistor-ladder.png)
...
| Supply | The board's own 3.3 V pin. On a 5 V board such as an Arduino Nano, 5 V works too |

Wiring:

1. Solder the resistors in a chain, end to end.
2. Connect one end of the chain to 3.3 V and the other end to GND.
3. Connect the switch's first position to the 3.3 V end, its last position to the GND end, and each position in between to the junction between two resistors, in order.
4. Connect the switch's common terminal to the analog input you will assign in the manager.

> [!WARNING]
> Never feed an analog input a voltage above the board's own supply. Power the chain from the board's own pins, not from an external supply.
...
In the setup wizard's **Pins** step, every rotary switch on an Open Hardware board offers a wiring choice. **One analog input** is the default. Pick the analog pin the common is wired to. The pin numbers are the ones printed on the board, so a Nano's analog inputs are A4 to A7 and a Pico's are GP26 to GP28.

![The Pins step with a rotary switch set to one analog input](images/ladder-wizard-wiring.png)

If you would rather wire the switch the classic way, change the choice to **One pin per position** and the row grows one pin slot per position.
...
The **Live** column in Console Panels follows the switch as you turn it. The calibration is kept with the board's setup in the manager, so it survives unplugging the board and moving it to another USB port.
